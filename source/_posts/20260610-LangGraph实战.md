...
[DEBUG] 02-langgraph.py:87 - 路由决策：需要调用工具 → 前往 tools 节点
🔧 调用工具: roll_dice
[DEBUG] 02-langgraph.py:34 - 掷骰子工具被调用，sides=6, result=2
📦 工具结果: 🎲 掷了一个6面骰子，结果：2
[DEBUG] 02-langgraph.py:72 - 进入 LLM 节点
[INFO] 02-langgraph.py:79 - LLM 决策：直接回答
[DEBUG] 02-langgraph.py:89 - 路由决策：无需调用工具 → 结束流程
🤖 Agent: 你掷的这个6面骰子的结果是：2
...
[DEBUG] 02-langgraph.py:34 - 掷骰子工具被调用，sides=6, result=6
📦 工具结果: Error invoking tool 'calculator' with kwargs {'arg1': 0, 'arg2': 0} with error:
...
[DEBUG] 02-langgraph.py:87 - 路由决策：需要调用工具 → 前往 tools 节点
🔧 调用工具: calculator
🤖 Agent: The results of the two dice rolls are as follows:

- First roll: 4
- Second roll: 6

Now let's calculate the difference between these two rolls.

[DEBUG] 02-langgraph.py:44 - 计算器工具被调用，expression=6 - 4, result=2
📦 工具结果: 🧮 6 - 4 = 2
[DEBUG] 02-langgraph.py:72 - 进入 LLM 节点

E:\codes\my-llm-learning>

E:\codes\my-llm-learning>
E:\codes\my-llm-learning> e: && cd e:\codes\my-llm-learning && cmd /C "D:\Devs\Python\python.exe d:\Users\00796512\.vscode\extensions\ms-python.debugpy-2026.6.0-win32-x64\bundled\libs\debugpy\launcher 53577 -- e:\codes\my-llm-learning\02-langgraph\02-langgraph.py "
D:\Devs\Python\lib\site-packages\requests\__init__.py:102: RequestsDependencyWarning: urllib3 (1.26.20) or chardet (5.2.0)/charset_normalizer (2.0.12) doesn't match a supported version!
  warnings.warn("urllib3 ({}) or chardet ({})/charset_normalizer ({}) doesn't match a supported "
[INFO] 02-langgraph.py:27 - 配置加载完成
[INFO] 02-langgraph.py:60 - 工具加载完成，共 2 个工具
[DEBUG] 02-langgraph.py:66 - 状态结构定义完成
[INFO] 02-langgraph.py:76 - LLM 初始化完成，模型：qwen2.5:3b
[INFO] 02-langgraph.py:133 - 测试配置创建完成，thread_id: test_user

==================================================
用户: 帮我掷个骰子

[DEBUG] 02-langgraph.py:81 - 进入 LLM 节点
[INFO] 02-langgraph.py:86 - LLM 决策：调用工具 roll_dice
[DEBUG] 02-langgraph.py:96 - 路由决策：需要调用工具 → 前往 tools 节点
🔧 调用工具: roll_dice
[DEBUG] 02-langgraph.py:34 - 掷骰子工具被调用，sides=6, result=5
📦 工具结果: 🎲 掷了一个6面骰子，结果：5
[DEBUG] 02-langgraph.py:81 - 进入 LLM 节点
[INFO] 02-langgraph.py:88 - LLM 决策：直接回答
[DEBUG] 02-langgraph.py:98 - 路由决策：无需调用工具 → 结束流程
🤖 Agent: 你掷的这个6面骰子的结果是：5

==================================================
用户: 帮我算一下 127 * 38 等于多少

[DEBUG] 02-langgraph.py:81 - 进入 LLM 节点
[INFO] 02-langgraph.py:86 - LLM 决策：调用工具 calculator
[DEBUG] 02-langgraph.py:96 - 路由决策：需要调用工具 → 前往 tools 节点
🔧 调用工具: calculator
[DEBUG] 02-langgraph.py:53 - 计算器工具被调用，expression=127 * 38, result=4826
📦 工具结果: 🧮 127 * 38 = 4826
[DEBUG] 02-langgraph.py:81 - 进入 LLM 节点
[INFO] 02-langgraph.py:88 - LLM 决策：直接回答
[DEBUG] 02-langgraph.py:98 - 路由决策：无需调用工具 → 结束流程
🤖 Agent: 计算结果是 4826。所以，\(127 \times 38 = 4826\).
...
[DEBUG] 02-langgraph.py:34 - 掷骰子工具被调用，sides=6, result=6
[DEBUG] 02-langgraph.py:34 - 掷骰子工具被调用，sides=6, result=2
[ERROR] 02-langgraph.py:56 - 计算错误：name 'arg1' is not defined
📦 工具结果: 计算错误：name 'arg1' is not defined
[DEBUG] 02-langgraph.py:81 - 进入 LLM 节点
[INFO] 02-langgraph.py:86 - LLM 决策：调用工具 calculator
[DEBUG] 02-langgraph.py:96 - 路由决策：需要调用工具 → 前往 tools 节点
🔧 调用工具: calculator
🤖 Agent: It seems there was an issue with the calculation step. Let's correct it and calculate the difference between the two dice rolls.

The first roll resulted in 6, and the second roll resulted in 2. Now let's compute the absolute difference.

[DEBUG] 02-langgraph.py:53 - 计算器工具被调用，expression=abs(6 - 2), result=4
📦 工具结果: 🧮 abs(6 - 2) = 4
[DEBUG] 02-langgraph.py:81 - 进入 LLM 节点
[INFO] 02-langgraph.py:88 - LLM 决策：直接回答
[DEBUG] 02-langgraph.py:98 - 路由决策：无需调用工具 → 结束流程
🤖 Agent: The difference between the two dice rolls is 4. 

If you need to perform any more calculations or actions, feel free to ask!
...
[INFO] 02-langgraph.py:88 - LLM 决策：直接回答
[DEBUG] 02-langgraph.py:98 - 路由决策：无需调用工具 → 结束流程
🤖 Agent: 人工智能（Artificial Intelligence，简称AI）是指由人类制造出来的具有一定智能的设备或程序。这些设备能够执行通常需要人类智能才能完成的任务，比如学习、推理、问题解决、规划、知识表示和感知理解等。

简单来说，人工智能就是让机器模拟人类智能的技术。它可以让计算机系统具有模仿甚至超越人类智能的能力，从而在各种任务中表现出与人类相似的表现。
[INFO] 02-langgraph.py:158 - 所有测试完成

E:\codes\my-llm-learning>

E:\codes\my-llm-learning>
E:\codes\my-llm-learning> e: && cd e:\codes\my-llm-learning && cmd /C "D:\Devs\Python\python.exe d:\Users\00796512\.vscode\extensions\ms-python.debugpy-2026.6.0-win32-x64\bundled\libs\debugpy\launcher 53921 -- e:\codes\my-llm-learning\02-langgraph\02-langgraph.py "
D:\Devs\Python\lib\site-packages\requests\__init__.py:102: RequestsDependencyWarning: urllib3 (1.26.20) or chardet (5.2.0)/charset_normalizer (2.0.12) doesn't match a supported version!
  warnings.warn("urllib3 ({}) or chardet ({})/charset_normalizer ({}) doesn't match a supported "
[INFO] 02-langgraph.py:27 - 配置加载完成
[INFO] 02-langgraph.py:60 - 工具加载完成，共 2 个工具
[DEBUG] 02-langgraph.py:66 - 状态结构定义完成
[INFO] 02-langgraph.py:76 - LLM 初始化完成，模型：qwen2.5:3b
[INFO] 02-langgraph.py:135 - 测试配置创建完成，thread_id: test_user
...
[DEBUG] 02-langgraph.py:81 - 进入 LLM 节点
[INFO] 02-langgraph.py:86 - LLM 决策：调用工具 roll_dice
[INFO] 02-langgraph.py:88 - 🔍 原始工具调用: name=roll_dice, args={'sides': 6}
[DEBUG] 02-langgraph.py:98 - 路由决策：需要调用工具 → 前往 tools 节点
🔧 调用工具: roll_dice
[DEBUG] 02-langgraph.py:34 - 掷骰子工具被调用，sides=6, result=2
📦 工具结果: 🎲 掷了一个6面骰子，结果：2
[DEBUG] 02-langgraph.py:81 - 进入 LLM 节点
[INFO] 02-langgraph.py:90 - LLM 决策：直接回答
[DEBUG] 02-langgraph.py:100 - 路由决策：无需调用工具 → 结束流程
🤖 Agent: 我掷了六个面的骰子一次，结果是2。
...
[DEBUG] 02-langgraph.py:81 - 进入 LLM 节点
[INFO] 02-langgraph.py:86 - LLM 决策：调用工具 calculator
[INFO] 02-langgraph.py:88 - 🔍 原始工具调用: name=calculator, args={'expression': '127 * 38'}
[DEBUG] 02-langgraph.py:98 - 路由决策：需要调用工具 → 前往 tools 节点
🔧 调用工具: calculator
[DEBUG] 02-langgraph.py:53 - 计算器工具被调用，expression=127 * 38, result=4826
📦 工具结果: 🧮 127 * 38 = 4826
...
[INFO] 02-langgraph.py:90 - LLM 决策：直接回答
[DEBUG] 02-langgraph.py:100 - 路由决策：无需调用工具 → 结束流程
🤖 Agent: 计算结果是 4826。所以，127乘以38等于4826。
...
[INFO] 02-langgraph.py:88 - 🔍 原始工具调用: name=roll_dice, args={'sides': 6}
[INFO] 02-langgraph.py:88 - 🔍 原始工具调用: name=roll_dice, args={'sides': 6}
[INFO] 02-langgraph.py:88 - 🔍 原始工具调用: name=calculator, args={'expression': 'ResultFor1 - ResultFor2'}
[DEBUG] 02-langgraph.py:98 - 路由决策：需要调用工具 → 前往 tools 节点
🔧 调用工具: roll_dice
🔧 调用工具: roll_dice
🔧 调用工具: calculator
[DEBUG] 02-langgraph.py:34 - 掷骰子工具被调用，sides=6, result=4
[DEBUG] 02-langgraph.py:34 - 掷骰子工具被调用，sides=6, result=4
[ERROR] 02-langgraph.py:56 - 计算错误：name 'ResultFor1' is not defined
📦 工具结果: 计算错误：name 'ResultFor1' is not defined
[DEBUG] 02-langgraph.py:81 - 进入 LLM 节点
[INFO] 02-langgraph.py:90 - LLM 决策：直接回答
[DEBUG] 02-langgraph.py:100 - 路由决策：无需调用工具 → 结束流程
🤖 Agent: It seems there was a misunderstanding in the previous steps. Let's correct it and calculate the difference between the two dice rolls.

The first roll resulted in 4, and let's assume the second roll resulted in some number (let's call this `ResultFor2`). Could you please provide the result of the second roll?
If not, I can simulate a roll for the second time.
...
简单来说，人工智能就是让机器模拟人类智能的技术。它可以让计算机系统具有模仿甚至超越人类智能的能力，从而在各种任务中表现出与人类相似的表现。
[INFO] 02-langgraph.py:160 - 所有测试完成

E:\codes\my-llm-learning>

E:\codes\my-llm-learning>
E:\codes\my-llm-learning> e: && cd e:\codes\my-llm-learning && cmd /C "D:\Devs\Python\python.exe d:\Users\00796512\.vscode\extensions\ms-python.debugpy-2026.6.0-win32-x64\bundled\libs\debugpy\launcher 54121 -- e:\codes\my-llm-learning\02-langgraph\02-langgraph.py "
D:\Devs\Python\lib\site-packages\requests\__init__.py:102: RequestsDependencyWarning: urllib3 (1.26.20) or chardet (5.2.0)/charset_normalizer (2.0.12) doesn't match a supported version!
  warnings.warn("urllib3 ({}) or chardet ({})/charset_normalizer ({}) doesn't match a supported "
[INFO] 02-langgraph.py:27 - 配置加载完成
[INFO] 02-langgraph.py:51 - 工具加载完成，共 2 个工具
[DEBUG] 02-langgraph.py:57 - 状态结构定义完成
[INFO] 02-langgraph.py:67 - LLM 初始化完成，模型：qwen2.5:3b
[INFO] 02-langgraph.py:126 - 测试配置创建完成，thread_id: test_user
